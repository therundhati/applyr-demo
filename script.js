...
function addDragListeners(card, job) {
  ...
  function onDrag(e) {
    ...
    // Overlay
    const threshold=100;
    if(offsetX>0) { greenOverlay.style.opacity = Math.min(offsetX/threshold,1); redOverlay.style.opacity=0; }
    else if(offsetX<0) { redOverlay.style.opacity = Math.min(Math.abs(offsetX)/threshold,1); greenOverlay.style.opacity=0; }
    else { redOverlay.style.opacity=0; greenOverlay.style.opacity=0; }
  }
